...
//       if (attempt > 0) {
//         console.log(
//           `[Gemini API] ${operationName} succeeded on attempt ${attempt + 1}`,
//         );
//       }
...
//       if (!shouldRetry) {
//         console.error(
//           `[Gemini API] ${operationName} failed (non-retryable or max retries reached):`,
//           error,
//         );
//         throw new Error(
//           `Gemini API ${operationName} failed: ${lastError.message}`,
//         );
//       }

//       console.warn(
//         `[Gemini API] ${operationName} failed on attempt ${attempt + 1}, retrying in ${delay}ms...`,
//         error.message || error,
//       );
...
//   throw new Error(
//     `Gemini API ${operationName} failed after ${RETRY_CONFIG.maxRetries + 1} attempts: ${lastError?.message}`,
//   );
// };

// export const makeGeminiCompletion = async (
//   prompt: string,
...
//   } catch (error) {
//     console.error(
//       `[Gemini API] Failed to parse JSON response for ${operationName}:`,
//       content,
//     );
//     throw new Error(
//       `Failed to parse Gemini API response for ${operationName}: Invalid JSON format`,
//     );
//   }
// };
...
//     const content = await makeGeminiCompletion(prompt, systemMessage);
//     return parseGeminiJsonResponse<IGeminiAnalysisResponse>(
//       content,
//       "Resume Analysis",
//     );
//   }, "Resume Analysis");
...
//   return callGeminiWithRetry(async () => {
//     const content = await makeGeminiCompletion(prompt, systemMessage);
//     return parseGeminiJsonResponse<IGeminiInterviewQuestionsResponse>(
//       content,
...
//   return callGeminiWithRetry(async () => {
//     const content = await makeGeminiCompletion(prompt, systemMessage);
//     return parseGeminiJsonResponse<IGeminiAnswerEvaluationResponse>(
//       content,
//       "Answer Evaluation",
//     );
//   }, "Answer Evaluation");
...
//   return callGeminiWithRetry(async () => {
//     const content = await makeGeminiCompletion(prompt, systemMessage);
//     const response = parseGeminiJsonResponse<IGeminiQuizResponse>(
//       content,
//       "Quiz Generation",
//     );
